...
int main (void){
  ...
  //////////          PAIR COUNTS          //////////
  ///////////////////////////////////////////////////
  matrix4d<double> DD(n_bin_perp, n_bin_pi, nReg, nReg);
  int _i;
...
#pragma omp atomic
                  DD(bin_perp, bin_pi, reg1, reg2) = DD(bin_perp, bin_pi, reg1, reg2) + gal.syst[_i]*gal.cp[_i]*gal.noz[_i]*gal.fkp[_i]*gal.syst[j]*gal.cp[j]*gal.noz[j]*gal.fkp[j];
                }
              }
            }	    
            j = lst[j];
    ...
    for (int j=0;j<n_bin_pi;j++) {
      for (int k=0;k<nReg;k++) {
        for (int l=0;l<nReg;l++) {
          if (DD(i,j,k,l)!=0) {
            pfw<<pow(10., i*log_dr_perp + log_r_perp_min)<<"\t"<<j*dr_pi<<"\t"<<k<<"\t"<<l<<"\t"<<setprecision(6)<<fixed<<DD(i,j,k,l)<<endl;
          }
        }
      }
    }
