...
int main (void){
  ...
  //////////          PAIR COUNTS          //////////
  ///////////////////////////////////////////////////
  matrix<double> RR(n_bin_perp, n_bin_pi);
  int i;
...
#pragma omp atomic
	          RR(bin_perp, bin_pi) = RR(bin_perp, bin_pi) + rand.syst[i]*rand.cp[i]*rand.noz[i]*rand.fkp[i]*rand.syst[j]*rand.cp[j]*rand.noz[j]*rand.fkp[j];
	        }
              }
	    }
	    j = lst[j];
  ...
  for (int i=0;i<n_bin_perp;i++)
    for (int j=0;j<n_bin_pi;j++)
      pfw<<pow(10., i*log_dr_perp + log_r_perp_min)<<"\t"<<j*dr_pi<<"\t"<<setprecision(6)<<fixed<<RR(i,j)<<endl;
  pfw.close();
