...
int main (void){
  ...
  //////////          PAIR COUNTS          //////////
  ///////////////////////////////////////////////////
  matrix4d<double> DR(n_bin_perp, n_bin_pi, nReg, nReg);
  int i;
...
#pragma omp atomic
	        DR(bin_perp, bin_pi, reg1, reg2) = DR(bin_perp, bin_pi, reg1, reg2) + gal.syst[i]*gal.cp[i]*gal.noz[i]*gal.fkp[i]*rnd.syst[j]*rnd.cp[j]*rnd.noz[j]*rnd.fkp[j];
	      }
            }
	    j = lst[j];
          }
    ...
    for (int j=0;j<n_bin_pi;j++) {
      for (int k=0;k<nReg;k++) {
        for (int l=0;l<nReg;l++) {
          if (DR(i,j,k,l)!=0) {
            pfw<<pow(10., i*log_dr_perp + log_r_perp_min)<<"\t"<<j*dr_pi<<"\t"<<k<<"\t"<<l<<"\t"<<setprecision(6)<<fixed<<DR(i,j,k,l)<<endl;
          }
        }
      }
    }
