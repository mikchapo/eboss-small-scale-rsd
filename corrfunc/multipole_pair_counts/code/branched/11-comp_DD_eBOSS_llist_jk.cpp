...
//  int nReg = 201;
  ...
  //////////          PAIR COUNTS          //////////
  ///////////////////////////////////////////////////
  cout<<"Bin Limits: "<<nBinsep<<" "<<nBinmu<<" "<<nReg<<" "<<nReg<<endl;
  matrix4d<double> DD(nBinsep, nBinmu, nReg, nReg);
  int _i;
          ...
          while (j!=0){
            if (_i<j) {
	          ...
	          int binmu = (int)((pair_mu-mumin)/dmu);
                  int reg1, reg2;
                  // cout<<"Bins "<<binsep<<" "<<binmu<<" "<<gal.jk[_i]<<" "<<gal.jk[j]<<endl;
                  if (gal.jk[_i]==-1 && gal.jk[j]==-1) {
                    reg1 = nReg-1;
                    reg2 = nReg-1;
                  } else if (gal.jk[_i]==-1) {
                    reg1 = nReg-1;
...
#pragma omp atomic
                  DD(binsep, binmu, reg1, reg2) = DD(binsep, binmu, reg1, reg2) + gal.syst[_i]*gal.cp[_i]*gal.noz[_i]*gal.fkp[_i]*gal.syst[j]*gal.cp[j]*gal.noz[j]*gal.fkp[j];
                }
              }
            }	    
            j = lst[j];
  ...
  cout<<"Pair Counts Complete"<<endl;
    ...
    for (int j=0;j<nBinmu;j++) {
      for (int k=0;k<nReg;k++) {
        for (int l=0;l<nReg;l++) {
          if (DD(i,j,k,l)!=0) {
            pfw<<pow(10., i*logDsep + logSepmin)<<"\t"<<j*dmu<<"\t"<<k<<"\t"<<l<<"\t"<<setprecision(6)<<fixed<<DD(i,j,k,l)<<endl;
          }
        }
      }
    }
    if (((i+1)%10)==0) {
      cout<<"Stored "<<i+1<<"th sep bin";
      print_time();
    }
  }
  pfw.close();

  cout<<"Output Stored"<<endl;
