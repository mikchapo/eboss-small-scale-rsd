...
//                    cout<<"Cat ID: "<<gal.id[i]<<" BW ID: "<<bw_ids[i]<<" BW_weight "<<is<<": "<<bw_weights(i,is)<<endl;
#pragma omp atomic
                    sum_fbr+=(int)(__builtin_popcount(bw_weights(i,is)));
                  }
//                  cout<<"sum_cov: "<<sum_cov<<" sum_fbr "<<sum_fbr<<endl;
                  double pip_weight = (double)(sum_cov)/(double)(sum_fbr);
#pragma omp atomic
	          DR(binsep,binmu) = DR(binsep,binmu) + gal.syst[i]*gal.noz[i]*gal.fkp[i]*rnd.syst[j]*rnd.cp[j]*rnd.noz[j]*rnd.fkp[j]*pip_weight*ang_weight;
	        }
              }
	      j = lst[j];
            }
